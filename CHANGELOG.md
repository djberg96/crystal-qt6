...
- Expanded `QWizard` with title/subtitle text-format control, side-widget and custom-button replacement, `next_id` and `size_hint`, default-property registration, and expanded `QWizardPage` coverage including lifecycle hooks, Qt-style setter aliases, and richer `register_field` overloads.
...
- Added `QTileRules` as a typed value wrapper with shared `TileRule` enum coverage and native conversion helpers for future tile-aware drawing and pixmap-style APIs.
...
- Added `QTableWidgetSelectionRange` as a typed value object with row and column span helpers, and integrated it into `QTableWidget` selection-range get/set workflows.
...
- Added `QStyleOptionGraphicsItem` with exposed-rect access and typed level-of-detail helpers for graphics-view paint state.
...
- Expanded `QSplitter` with insertion, replacement, per-pane collapsible control, refresh, size hints, saved-state roundtripping, range and handle lookup, stretch-factor control, and `splitter_moved`, and added a `QSplitterHandle` wrapper for handle orientation, size-hint, and parent-splitter access.
...
- Expanded `QStyleOptionViewItem` from a thin read-only delegate shell into a full item-view paint-state wrapper with typed alignment, elide, decoration, feature, check-state, icon, text, brush, widget, and model-index helpers.
...
- Expanded `QGraphicsWidget` with layout direction, style, focus policy, window-title, contents/window-frame margins, local rect, attribute, close, and richer geometry/sizing helpers for fuller graphics-view widget composition.
...
- Expanded `QGraphicsTransform` with shared matrix-application and update helpers, and added a `Matrix4x4` value type for graphics-view transform inspection.
