...
- `Qt6::ModelIndex`, `Qt6::AbstractItemModel`, `Qt6::AbstractListModel`, `Qt6::AbstractTreeModel`, `Qt6::StandardItem`, `Qt6::StandardItemModel`, `Qt6::SortFilterProxyModel`, `Qt6::StyledItemDelegate`, `Qt6::FileIconProvider`, `Qt6::FileSystemModel`, `Qt6::ListView`, `Qt6::TreeView`, and `Qt6::ColumnView` for a broader model/view layer with roles, mutable callback-backed list/tree models, live filesystem browsing, Finder-style column navigation, file icons plus permission/modified-time metadata, preview-update callbacks, row insert/remove/move notifications, proxy sorting/filtering with regex and recursive tree filtering, delegate-based display formatting, drag/drop MIME payloads, and shared view-side helpers like `scroll_to`, `select_all`, and drag/drop configuration
- richer text and table polish through `QTextDocument#find`, `QTextCursor` null/replace helpers, editor-side plain-text/HTML insertion helpers, `QKeySequenceEdit` shortcut capture, wizard-page mandatory field registration and shared field lookups, `QDataWidgetMapper` form binding, completer wrap/visibility controls, header section-size/interaction access, table sort/resize-to-contents helpers, and `TableWidget#on_item_double_clicked`
