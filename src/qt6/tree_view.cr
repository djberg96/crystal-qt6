module Qt6
  ...
  class TreeView < Widget
    ...
    def edit_triggers : EditTrigger
      EditTrigger.from_value(LibQt6.qt6cr_tree_view_edit_triggers(to_unsafe))
    end

    # Sets the edit triggers enabled for the view.
    def edit_triggers=(value : EditTrigger) : EditTrigger
      LibQt6.qt6cr_tree_view_set_edit_triggers(to_unsafe, value.value)
      value
    end
    ...
    def open_persistent_editor(index : ModelIndex) : self
      LibQt6.qt6cr_tree_view_open_persistent_editor(to_unsafe, index.to_unsafe)
      self
    end

    # Closes a persistent editor for the given index.
    def close_persistent_editor(index : ModelIndex) : self
      LibQt6.qt6cr_tree_view_close_persistent_editor(to_unsafe, index.to_unsafe)
      self
    end

    # Returns whether a persistent editor is open for the given index.
    def persistent_editor_open?(index : ModelIndex) : Bool
      LibQt6.qt6cr_tree_view_is_persistent_editor_open(to_unsafe, index.to_unsafe)
    end
