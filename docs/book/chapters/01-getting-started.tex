...
That depth is the reason \api{crystal-qt6} is useful. Instead of building a small custom UI layer and then discovering that file dialogs, model/view tables, undo stacks, text editing, or image rendering are missing, this binding grows around Qt's existing desktop vocabulary. The goal is not to hide Qt. The goal is to make Qt feel natural from Crystal.
...
Those targets are not special entry points into the binding. They wrap ordinary Crystal commands, so direct commands work as well:
