...
The repository's \api{make spec} target also applies the same Qt platform defaults that CI uses, so popup-oriented widget specs do not accidentally inherit a Wayland session or another desktop backend that changes window-manager behavior. A direct \api{crystal spec} run still uses whatever Qt platform backend your environment selects unless you override it yourself.
...
For local Linux automation, the CI command is a good starting point, and the repository's \api{make spec} target already follows this path:
