...
\api{ColorDialog} returns a \api{Color}. It is useful for theme colors, layer colors, brush colors, annotation colors, and any workflow where the user needs a visual picker. The convenience helper is usually enough, but explicit instances also expose option flags, the currently highlighted color, the accepted color, and callbacks for live preview or selection-confirmed flows.
...
\begin{lstlisting}[style=crystal]
dialog = Qt6::ColorDialog.new(main, accent)
dialog.window_title = "Tune Accent"
dialog.native_dialog = false
dialog.options = Qt6::ColorDialogOption::ShowAlphaChannel
dialog.set_option(Qt6::ColorDialogOption::NoEyeDropperButton)

dialog.on_current_color_changed do |preview|
  preview_swatch.style_sheet = "background: rgba(#{preview.red}, #{preview.green}, #{preview.blue}, #{preview.alpha});"
end

if dialog.exec == Qt6::DialogCode::Accepted
  accent = dialog.selected_color
end

dialog.release
\end{lstlisting}

\api{ColorDialog#native_dialog=} can disable the platform-native picker. That is mainly useful for deterministic tests, where the widget-backed dialog behaves like an ordinary Qt dialog. The binding also exposes \api{options}, \api{set_option}, and \api{clear_option} for workflows that need alpha editing, no-button live-preview modes, or other picker configuration.

Qt also keeps shared custom and standard color slots. \api{ColorDialog.custom_color}, \api{set_custom_color}, \api{standard_color}, and \api{set_standard_color} are useful when an application wants to seed the picker with a house palette, terrain swatches, or other reusable colors.
...
\api{InputDialog} is a compact way to request one value from the user. The convenience helpers cover text, multiline text, integers, floating-point values, and item choices.
...
\begin{lstlisting}[style=crystal]
notes = Qt6::InputDialog.get_multi_line_text(
  main,
  title: "Export Notes",
  label: "Summary",
  value: "Tiles ready for review."
)
\end{lstlisting}
...
Use explicit \api{InputDialog} instances when you need to change the input mode, inspect range values, adjust integer steps or floating-point decimals, customize button labels, switch text echo modes, or listen to value-changed and value-selected signals directly.
...
Splash screens should stay brief. If the user might need to cancel, inspect details, or wait for a longer task, prefer a \api{ProgressDialog} instead. The older \api{show_message("Loading...", color)} form still works when explicit alignment is unnecessary.
...
buttons.center_buttons = true
buttons.orientation = Qt6::Orientation::Horizontal
...
The button box can also be reconfigured after construction through \api{standard_buttons=}. That is useful when a dialog switches between an ordinary Ok/Cancel footer and a more wizard-like Apply/Close footer without rebuilding the surrounding layout.
