...
As with decoding, exact writable formats depend on the Qt plugins in the deployment. The current binding exposes direct \api{save} and \api{save_to_data} operations, so choose the path extension or explicit format, then check the boolean result. If a later binding release adds a \api{QImageWriter} wrapper, that is where writer-specific quality, compression, progressive encoding, and metadata options would belong.
