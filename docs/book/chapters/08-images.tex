...
Exact format support depends on the Qt image plugins available in the user's installation. File extensions and filters are useful hints, but \api{QImageReader#can_read?} is the source of truth for a particular file or device. When loading fails, show \api{error_string} so the user sees the platform's real decoder error.

\section{A Small Import/Export Flow}

The following flow is close to what a small editor or preview panel needs: probe the selected file, decode with orientation handling, keep the working copy as a \api{QImage}, convert to \api{QPixmap} only for display, and check the save result when exporting.

\begin{lstlisting}[style=crystal]
input_path = "assets/source-photo.jpg"
output_path = "preview-export.png"

reader = Qt6::QImageReader.new(input_path)
reader.auto_transform = true

if reader.can_read?
  original = reader.read

  unless original.null?
    working = original.scaled(
      320,
      220,
      Qt6::AspectRatioMode::Keep,
      Qt6::TransformationMode::Smooth
    ...
    preview.pixmap = working.to_pixmap

    unless working.save(output_path)
      status_bar.show_message("Could not save #{output_path}", 2200)
    end
  end
else
  status_bar.show_message(reader.error_string, 2200)
...
For a widget screenshot export, the same pattern happens at the UI boundary: grab a \api{QPixmap}, save it, and check the return value.

\begin{lstlisting}[style=crystal]
pixmap = preview.grab
unless pixmap.save("preview-widget.png")
  status_bar.show_message("Could not export widget preview", 2200)
end
\end{lstlisting}
...
As with decoding, exact writable formats depend on the Qt plugins in the deployment. For simple exports, \api{save} and \api{save_to_data} are enough: choose the path extension or explicit format, then check the boolean result. Use \api{QImageWriter} when the export needs writer-specific options such as quality, compression, optimized output, or progressive encoding.

\begin{lstlisting}[style=crystal]
writer = Qt6::QImageWriter.new("preview.jpg", "jpg")
writer.quality = 92
writer.optimized_write = true

unless writer.can_write? && writer.write(image)
  status_bar.show_message(writer.error_string, 2200)
end
\end{lstlisting}

\api{QImageWriter} also works with an open \api{IODevice}, which is useful when encoded images should stay in memory:

\begin{lstlisting}[style=crystal]
bytes = Qt6::QByteArray.new
buffer = Qt6::QBuffer.new(bytes)
buffer.open(Qt6::IODeviceOpenMode::WriteOnly)

writer = Qt6::QImageWriter.new(buffer, "png")
...
The common image bugs are usually simple mismatches between the data representation and the job at hand:

\begin{itemize}
\item Check \api{null?}, \api{can_read?}, and save return values. A failed decode can still give you an object, but not useful pixels.
\item Be precise about raw byte order. For \api{ImageFormat::ARGB32} on little-endian systems, the bytes are commonly blue, green, red, alpha.
\item Do not use \api{AspectRatioMode::Ignore} for thumbnails unless stretching is truly intended. Use \api{Keep} for previews that should preserve the source shape.
\item Keep the working image as a \api{QImage} while you still need pixel access, raw bytes, format conversion, or encoded data. Convert to \api{QPixmap} at the widget boundary.
\end{itemize}
...
Treat image format support as deployment-specific. Test with the plugins your application ships, and check \api{QImageReader#can_read?} or save return values for the exact file at hand.
