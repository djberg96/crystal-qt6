...
Dock visibility belongs in the View menu. When you need a simple dock toggle, use a checkable action and connect it to the dock's visible state:

\begin{lstlisting}[style=crystal]
inspector_action = Qt6::Action.new("Inspector", main)
inspector_action.checkable = true
inspector_action.checked = properties_dock.visible?
inspector_action.on_toggled do |visible|
  properties_dock.visible = visible
end

view_menu << inspector_action
\end{lstlisting}

If a future wrapper exposes a dock-owned toggle action, prefer that for automatic synchronization with user-driven dock closing and showing. The explicit action pattern above is still useful for application-owned visibility rules.
